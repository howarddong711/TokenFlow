...
use crate::core::{
    OAuthCredentials, ProviderError, ProviderFetchResult, RateWindow, UsageSnapshot,
};
...
impl ClaudeOAuthFetcher {
    ...
    pub async fn fetch_with_credentials(
        ...
        let credentials = Self::to_claude_credentials(credentials);
        let usage_response = self.fetch_usage(&credentials).await?;
        let usage = self.build_usage_snapshot(&usage_response, &credentials);
        Ok(ProviderFetchResult::new(usage, "oauth"))
    }

    pub fn load_core_credentials(&self) -> Result<OAuthCredentials, ProviderError> {
        let credentials = self.load_credentials()?;
        Ok(Self::to_core_credentials(&credentials))
    }

    pub fn credentials_fingerprint(&self) -> Result<Option<String>, ProviderError> {
    ...
    fn to_core_credentials(credentials: &ClaudeOAuthCredentials) -> OAuthCredentials {
        OAuthCredentials {
    ...
    fn to_claude_credentials(credentials: &OAuthCredentials) -> ClaudeOAuthCredentials {
