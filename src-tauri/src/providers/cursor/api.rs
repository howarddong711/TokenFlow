...
impl CursorApi {
    ...
    pub async fn fetch_usage(
        ...
        ProviderError,
    > {
        self.fetch_usage_with_cookie(None).await
    }

    /// Fetch usage with an optional externally-provided cookie header.
    /// When `external_cookie` is Some, it is used directly instead of
    /// extracting cookies from the browser.
    pub async fn fetch_usage_with_cookie(
        &self,
        external_cookie: Option<&str>,
    ) -> Result<
        (
            RateWindow,
            Option<CostSnapshot>,
        ...
        ProviderError,
    > {
        // Use the provided cookie or fall back to browser extraction
        let cookie_header = match external_cookie {
            Some(c) => c.to_string(),
            None => self.get_cookie_header()?,
        };
