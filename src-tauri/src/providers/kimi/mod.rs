...
impl KimiProvider {
    ...
    /// Extract auth token from a cookie header string.
    /// Used for multi-account support with externally-provided cookies.
    fn extract_token_from_cookies(cookies: &str) -> Result<String, ProviderError> {
        if cookies.is_empty() {
            return Err(ProviderError::AuthRequired);
        }
    ...
    async fn fetch_via_web(&self, ctx: &FetchContext) -> Result<UsageSnapshot, ProviderError> {
        // Use manual cookie header if provided (multi-account support),
        // otherwise extract from browser
        let token = match ctx.manual_cookie_header.as_deref() {
            Some(cookie_str) => {
                // Extract auth token from the provided cookie header
                Self::extract_token_from_cookies(cookie_str)?
            }
