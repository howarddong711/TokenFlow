...
impl BrowserDetector {
    ...
    fn detect_profiles(browser_type: BrowserType, user_data_dir: &PathBuf) -> Vec<BrowserProfile> {
        if browser_type == BrowserType::Cursor {
            return Self::detect_cursor_profiles(user_data_dir);
        }
    ...
    /// Detect Cursor desktop profiles.
    /// Cursor stores its Chromium network cookies directly in the app data root.
    fn detect_cursor_profiles(user_data_dir: &PathBuf) -> Vec<BrowserProfile> {
        let cookies_path = user_data_dir.join("Network").join("Cookies");
