...
impl CookieExtractor {
    ...
    pub(crate) fn extract_profile_cookies(
        browser: &DetectedBrowser,
...
/// A browser+profile label and its associated cookie header.
/// Used for multi-account support: each browser/profile combo may represent
/// a different logged-in account for the same provider.
#[derive(Debug, Clone)]
pub struct BrowserCookieSet {
    /// Human-readable label like "Chrome/Default", "Edge/Profile 1"
    pub browser_label: String,
    /// The cookie header string for HTTP requests
    pub cookie_header: String,
}

/// Get cookie headers from ALL browser+profile combinations for a domain.
/// Returns one entry per browser/profile that has cookies for the given domain.
/// This enables multi-account support: different browsers/profiles may be
/// logged into different accounts on the same provider.
pub fn get_all_cookie_headers_by_profile(domain: &str) -> Vec<BrowserCookieSet> {
    use super::detection::BrowserDetector;
    ...
    for browser in &browsers {
        for profile in &browser.profiles {
            match CookieExtractor::extract_profile_cookies(browser, profile, domain) {
                Ok(cookies) if !cookies.is_empty() => {
                    let cookie_header = CookieExtractor::build_cookie_header(&cookies);
                    let browser_label =
                        format!("{}/{}", browser.browser_type.display_name(), profile.name);
                    tracing::debug!(
                        "Found {} cookies for {} in {}",
                        cookies.len(),
                        domain,
